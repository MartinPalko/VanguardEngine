...
namespace Vanguard
{
	ManagedAssembly::ManagedAssembly(const String& aMonoBinaryName)
	{
		...
		FileSystem::SetCurrentWorkingDir(Directories::GetEngineBinDirectory());

		Log::Message("Mono Initializing","Mono");

		mono_config_parse(NULL);
		mono_set_dirs(String(Directories::GetEngineBinDirectory().GetFullPathName() + "/mono/lib/").GetCharPointer(), String(Directories::GetEngineBinDirectory().GetFullPathName() + "/mono/lib/").GetCharPointer());
